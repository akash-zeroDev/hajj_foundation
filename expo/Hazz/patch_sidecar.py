...
# 2. Add the Download button next to X button
...
new_header_btns = """          <div className="absolute top-[16px] right-[16px] flex items-center gap-2 z-10">
            {onDownloadStatement && (
              <button 
...
# 3. Update the bottom buttons
old_bottom = """        {(onRemove || onDownloadStatement) && !activeMember.isRemoved && (
          <div className="p-[20px_24px_24px] bg-white grid gap-[10px] shrink-0 border-t border-slate-100">
            ...
            {onRemove && (
              ...
              >
                {isRemoving ? (
                  <span className="w-[14px] h-[14px] border-2 border-red-600 border-t-transparent rounded-full animate-spin"></span>
                ) : (
